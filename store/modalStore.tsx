...
export const useModalStore = create<ModalState>((set) => ({
  ...
  showModal: (modal) =>
    set((state) => {
      ...
      const incomingType = (modal.content as any)?.type?.name || (modal.content as any)?.type?.displayName
      if (incomingType) {
        const alreadyOpen = state.modals.some((m) => {
          const t = (m.content as any)?.type?.name || (m.content as any)?.type?.displayName
