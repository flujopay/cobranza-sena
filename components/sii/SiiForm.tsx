  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <p className="text-sm text-gray-500">
        Tu empresa está autenticada con el SII. Las credenciales se almacenan encriptadas en nuestros
        servidores.
      </p>
      ...
      {!confirm ? (
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex flex-col gap-3">
          <p className="text-sm text-red-700 font-medium">
            ¿Seguro que deseas eliminar las credenciales SII?
          </p>
          <p className="text-xs text-red-500">
            Esta acción no se puede deshacer. Deberás volver a ingresar tus credenciales para sincronizar
            documentos.
          </p>
...
export function SiiForm() {
  ...
  async function onSubmit(data: SiiFields) {
    ...
    try {
      ...
      showToast({
        message: 'SII conectado',
        subMessage: 'Credenciales guardadas correctamente.',
        iconType: 'success',
      })
    } catch (err) {
  ...
  async function handleDisconnect() {
    try {
      ...
      showToast({
        message: 'SII desconectado',
        subMessage: 'Las credenciales fueron eliminadas.',
        iconType: 'success',
      })
    } catch (err) {
