...
function renderPreview(cuerpo: string): string {
  return Object.entries(PREVIEW_VARS).reduce((text, [key, val]) => text.replaceAll(key, val), cuerpo)
}
      ...
      <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span
              className={`inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full border ${RIESGO_COLORS[plantilla.nivelRiesgo]}`}
            >
          ...
          {asuntoPreview && <p className="text-xs text-gray-500 mt-0.5 truncate">Asunto: {asuntoPreview}</p>}
        </div>
        <button
