...
const columns: Column<Contact>[] = [
  ...
  },
  {
    ...
    header: 'Teléfono',
    render: (r) =>
      r.phone ? <span className="text-xs">{r.phone}</span> : <span className="text-gray-300 text-xs">—</span>,
  },
  {
    key: 'ruc',
    header: 'RUT',
  ...
  },
  {
    ...
    render: (r) => (
      <div className="flex flex-col gap-0.5 max-w-[200px]">
        <span className="text-sm text-gray-800 truncate" title={r.client_name ?? ''}>
          {r.client_name || '—'}
        </span>
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {count > 0
            ? `${count.toLocaleString('es-CL')} contactos registrados.`
            : 'Personas de contacto por cliente para gestionar la cobranza.'}
        </p>
      </div>
