...
function StateBadge({ state }: { state: string | null }) {
  ...
  return (
    <span
      className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${cls}`}
    >
      {state}
    </span>
  )
...
const columns: Column<Invoice>[] = [
  ...
  },
  {
    ...
    render: (r) => (
      <div className="flex flex-col gap-0.5 max-w-[200px]">
        ...
        {r.receiver_ruc && <span className="font-mono text-[11px] text-gray-400">{r.receiver_ruc}</span>}
      </div>
    ),
  ...
  },
  {
    ...
    header: 'Emisión',
    render: (r) => (
      <span className="text-xs text-gray-600 whitespace-nowrap">{formatDate(r.issue_date)}</span>
    ),
  ...
  },
  {
    ...
    render: (r) => {
      ...
      return (
        <span
          className={`text-xs whitespace-nowrap ${isPast && hasPending ? 'text-red-600 font-medium' : 'text-gray-600'}`}
        >
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {count > 0
            ? `${count.toLocaleString('es-CL')} facturas importadas desde el SII.`
            : 'Facturas emitidas importadas desde el SII.'}
        </p>
      </div>
