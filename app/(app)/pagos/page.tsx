...
const columns: Column<Payment>[] = [
  {
    ...
    render: (r) => (
      <div className="flex flex-col gap-0.5 max-w-[200px]">
        <span className="font-medium text-gray-900 truncate" title={r.client_name ?? ''}>
          {r.client_name || '—'}
        </span>
        {r.client_ruc && <span className="font-mono text-[11px] text-gray-400">{r.client_ruc}</span>}
      </div>
    ),
  ...
  },
  {
    ...
    render: (r) => (
      <span className="text-xs text-gray-600 whitespace-nowrap">{formatDate(r.payment_date_estimated)}</span>
    ),
  ...
  },
  {
    ...
    render: (r) =>
      r.email ? (
        <a href={`mailto:${r.email}`} className="text-blue-600 hover:underline text-xs">
          {r.email}
        </a>
      ) : (
        <span className="text-gray-300 text-xs">—</span>
  ...
  },
  {
    ...
    render: (r) => (
      <div className="flex flex-col gap-0.5">
        ...
        {r.created_by_email && <span className="text-[11px] text-gray-400">{r.created_by_email}</span>}
      </div>
    ),
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {count > 0
            ? `${count.toLocaleString('es-CL')} pagos registrados.`
            : 'Pagos recibidos de clientes y su estado de validación.'}
        </p>
      </div>
