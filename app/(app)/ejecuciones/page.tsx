...
function StatusBadge({ status }: { status: CollectionRun['status'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2 py-0.5 rounded-full ${cls}`}
    >
      {status === 'running' && (
...
const columns: Column<CollectionRun>[] = [
  ...
  },
  {
    ...
    render: (r) =>
      r.error_message ? (
        <span className="text-xs text-red-600 truncate max-w-[200px] block" title={r.error_message}>
          {r.error_message}
        </span>
      ) : (
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Historial de ejecuciones del agente de cobranza.</p>
      </div>
