  ...
  try {
    ...
    if (err instanceof ApiError) {
      ...
      if (code === 'PASSWORD_LOGIN_DISABLED')
        return { errors: { general: 'Esta cuenta usa inicio de sesión con Google o Microsoft.' } }
    }
...
export async function registerAction(_prev: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  try {
    ...
    if (err instanceof ApiError) {
      const code = err.body.error_code
      if (code === 'RUT_ALREADY_EXISTS')
        return { errors: { rut: 'Ya existe una empresa registrada con este RUT.' } }
